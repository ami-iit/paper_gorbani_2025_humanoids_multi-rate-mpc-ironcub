...
void ReferenceTrackingCost::configureDynVectorsSize(QPInput& qpInput)
{
    ...
    m_Q.block(positionErrorIdx[0],
              positionErrorIdx[0],
              positionErrorIdx.size(),
              positionErrorIdx.size())
        = m_weightCoMPosError.asDiagonal();
    m_Q.block(rpyErrorIdx[0], rpyErrorIdx[0], rpyErrorIdx.size(), rpyErrorIdx.size())
        = m_weightRPYError.asDiagonal();
    m_stateReference.resize(m_nStates, m_nIter);
    m_stateReference.setZero();
    m_positionCoMReference.resize(3, m_nIter - m_nIterSmall + 1);
    m_linearMomentumReference.resize(3, m_nIter - m_nIterSmall + 1);
    m_RPYReference.resize(3, m_nIter - m_nIterSmall + 1);
    m_angularMomentumReference.resize(3, m_nIter - m_nIterSmall + 1);
    ...
    for (int i = 0; i < (m_nIter - m_nIterSmall + 1); i++)
    {
...
{
    if (CoMPosRef.rows() != 3 || CoMPosRef.cols() != m_nIter - m_nIterSmall + 1)
    {
    ...
    for (int i = 0; i < m_nIter; i++)
    {
        if (i < m_nIterSmall)
        {
            m_stateReference.block(CoMPosIdx[0], i, 3, 1) = CoMPosRef.col(0);
        } else
        {
            m_stateReference.block(CoMPosIdx[0], i, 3, 1) = CoMPosRef.col(i - m_nIterSmall);
        }
    }
    return true;
}
...
{
    if (linMomRef.rows() != 3 || linMomRef.cols() != m_nIter - m_nIterSmall + 1)
    {
        yError() << "The size of the input vector is not correct.";
        return false;
    }
    for (int i = 0; i < m_nIter; i++)
    {
        if (i < m_nIterSmall)
        {
            m_stateReference.block(linMomIdx[0], i, 3, 1) = linMomRef.col(0);
        } else
        {
            m_stateReference.block(linMomIdx[0], i, 3, 1) = linMomRef.col(i - m_nIterSmall);
        }
    }
    return true;
}
...
{
    if (RPYRef.rows() != 3 || RPYRef.cols() != m_nIter - m_nIterSmall + 1)
    {
        yError() << "The size of the input vector is not correct.";
        return false;
    }
    for (int i = 0; i < m_nIter; i++)
    {
        if (i < m_nIterSmall)
        {
            m_stateReference.block(rpyIdx[0], i, 3, 1) = RPYRef.col(0);
        } else
        {
            m_stateReference.block(rpyIdx[0], i, 3, 1) = RPYRef.col(i - m_nIterSmall);
        }
    }
    return true;
}
...
{
    if (angMomRef.rows() != 3 || angMomRef.cols() != m_nIter - m_nIterSmall + 1)
    {
        yError() << "The size of the input vector is not correct.";
        return false;
    }
    for (int i = 0; i < m_nIter; i++)
    {
        if (i < m_nIterSmall)
        {
            m_stateReference.block(angMomIdx[0], i, 3, 1) = angMomRef.col(0);
        } else
        {
            m_stateReference.block(angMomIdx[0], i, 3, 1) = angMomRef.col(i - m_nIterSmall);
        }
    }
    return true;
}
...
const bool ThrottleInitialValueCost::computeHessianAndGradient(QPInput& qpInput)
{
    ...
    for (int i = 0; i < m_nJets; i++)
    {
        m_gradient(m_nStates * (m_nIter + 1) + m_nCtrlJoints * m_ctrlHorizon + i)
            = -m_weightThrottle
              * m_jetModel->compute_v(
                  m_jetModel->standardizeThrottle_u2T(qpInput.getThrottleMPC()[i]));
    }
    return true;
}
...
JointPositionRegularizationCost::JointPositionRegularizationCost(const unsigned int nVar,
                                                                 const unsigned int nState,
                                                                 const unsigned int nJoints)
    : IQPCost(nVar)
{
...
const bool JointPositionRegularizationCost::readConfigParameters(
    std::weak_ptr<BipedalLocomotion::ParametersHandler::IParametersHandler> parametersHandler,
    QPInput& qpInput)
{
    auto ptr = parametersHandler.lock();
    ...
    if (!ptr->getParameter("weightRegularizationJointPos", m_weightJointPos))
    {
        yError() << "Parameter 'weightRegularizationJointPos' not found in the config file.";
        return false;
    }
    if (!ptr->getParameter("controlHorizon", m_ctrlHorizon))
    {
    ...
    if (!ptr->getParameter("controlledJoints", m_ctrlJointsNames))
    {
    ...
    m_jointPosReference.resize(m_nCtrlJoints);
    for (int i = 0; i < m_nCtrlJoints; i++)
    {
        for (int j = 0; j < m_robot->getNJoints(); j++)
        {
            if (m_ctrlJointsNames[i] == m_robot->getJointName(j))
            {
                m_jointPosReference(i) = m_robot->getJointPos()[j];
                break;
            }
        }
    }
...
void JointPositionRegularizationCost::configureDynVectorsSize(QPInput& qpInput)
{
}

const bool JointPositionRegularizationCost::computeHessianAndGradient(QPInput& qpInput)
{
    if (m_firstUpdate)
    {
        m_hessian.setZero();
        ...
        for (int i = 0; i < m_ctrlHorizon; i++)
        {
            m_hessian.block(m_nStates * (m_nIter + 1) + i * m_nCtrlJoints,
                            ...
                            m_nCtrlJoints,
                            m_nCtrlJoints)
                = m_weightJointPos * Eigen::MatrixXd::Identity(m_nCtrlJoints, m_nCtrlJoints);
        }
        m_firstUpdate = false;
    }
    Eigen::VectorXd jointPos(m_nCtrlJoints);
    for (int i = 0; i < m_nCtrlJoints; i++)
    {
        for (int j = 0; j < m_robot->getNJoints(); j++)
        {
            if (m_ctrlJointsNames[i] == m_robot->getJointName(j))
            {
                jointPos(i) = qpInput.getOutputQPJointsPosition()[j];
                break;
            }
        }
    }
    for (int i = 0; i < m_ctrlHorizon; i++)
    {
        m_gradient.segment(m_nStates * (m_nIter + 1) + i * m_nCtrlJoints, m_nCtrlJoints)
            = m_weightJointPos * (jointPos - m_jointPosReference);
    }
    return true;
}

const bool JointPositionRegularizationCost::configureVectorsCollectionServer(QPInput& qpInput)
{
    return true;
}

const bool JointPositionRegularizationCost::populateVectorsCollection(
